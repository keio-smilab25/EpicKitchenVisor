...
def generate_masks_for_image(
    ...
    if not np.all(img == 0):
        ...
        if is_overlay:
            ...
            video = "_".join(image_name.split("_")[:1])
...
def folder_of_jsons_to_masks(
    ...
    for json_file in tqdm(sorted(glob.glob(os.path.join(json_files_path, "*.json")))):
        if "interpolation" in os.path.basename(json_file):
            ...
            input_resolution = (1920, 1080)
            frame_rate = 3
        objects_keys = json_to_masks_new(
            json_file,
            output_directory,
            is_overlay=is_overlay,
            images_root_directory=rgb_frames,
        ...
        if generate_video:
            generate_video_from_images(
                os.path.join(
                    output_directory,
                    "_".join(os.path.basename(json_file).split(".")[0].split("_")[:2]),
                ),
                frame_rate,
            )
        data = pd.DataFrame(
            objects_keys.items(), columns=["object_name", "unique_index"]
        )
        ...
        if not os.path.isfile(os.path.join(output_directory, "data_mapping.csv")):
            data.to_csv(
                os.path.join(output_directory, "data_mapping.csv"),
                index=False,
                header=["video_id", "object_name", "unique_index"],
            )
        else:
            data.to_csv(
                os.path.join(output_directory, "data_mapping.csv"),
...
def generate_video_from_images(input_directory, frame_rate):
    ...
    x = (
        "ffmpeg -y -pattern_type glob -r "
        + str(frame_rate)
        + " -i '"
        + input_directory
        + "/*.png' -vcodec mpeg4 -qmin 1 -qscale:v 1 "
        + out_file
        + ".mp4"
    )
    os.system(x)
